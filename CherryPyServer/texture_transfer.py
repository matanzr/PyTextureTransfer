...
def get_matching_block2(img, # image being synthesized
                       src, # source image
                       target, # target image
    ...
    locations = probe.all_locations()
    img_block = get_block(img, topLeft, blockWidth, blockHeight)
    target_block = get_block(target, topLeft, blockWidth, blockHeight)
    samples, d = compare_blocks(img_block, target_block, src, locations, alpha,
                    isFirstIteration, overlapX, overlapY, blockWidth, blockHeight)

    min = np.argsort(d)[:5]
    locations = get_neighbors(locations[min], 3, probe.width, probe.height)

    samples, d = compare_blocks(img_block, target_block, src, locations, alpha,
                    isFirstIteration, overlapX, overlapY, blockWidth, blockHeight)

    min = np.argmin(d)
    cut = get_block(img, topLeft, blockWidth, blockHeight) - samples[min]
    cut = np.sum(cut * cut, 2)
    return locations[min], cut[0: overlapY], cut[..., 0: overlapX]
...
def cut_and_paste(src, src_top_left, dest, dest_top_left, h_cut, v_cut, block_width, block_height, display_boundary_cut):
    ...
    #if not h_cut is None:
    #    s[h_cut.astype(np.int), range(h_cut.shape[0]),:] = [255,0,0]
